            ...
            {["All", ...categories].map((category) => (
              <button onClick={() => setSelectedCategory(category)} key={category} className="relative md:px-6 px-4 py-2 rounded-full cursor-pointer">
                ...
                {selectedCategory === category && (
                  <motion.div style={{borderRadius: 9999, boxShadow: "var(--inset_shadow)"}} transition={{type: "spring", stiffness: 420, damping: 24}} className="bg-white/10 light:bg-black/10 absolute inset-0 -z-1" layoutId="backGround_category" />
                )}
              </button>
            ))}
          </motion.div>
              ...
              <motion.div
                ...
                <Link href={project.link} className={`flex flex-col group md:hover:opacity-100 transition-opacity duration-300 ${index % 2 === 1 ? "md:translate-y-14" : ""}`}>
                  <div
                    ...
                    style={{boxShadow: "var(--inset_shadow)", backgroundColor: theme == "light" ? getBackgroundColor(project.title, 84, 50) : getBackgroundColor(project.title, 8, 0)}}>
