...
export default function Page() {
  ...
  return (
    <main className="min-h-screen max-w-7xl pt-32 mx-auto flex flex-col">
      ...
      <input type="range" value={brightness} onChange={(e) => setBrightness(Number(e.target.value))} className="w-32 cursor-pointer" />
      <p>{brightness}</p>
      <p className="pt-10">Pick a saturation</p>
      <input type="range" value={saturation} onChange={(e) => setSaturation(Number(e.target.value))} className="w-32 cursor-pointer" />
      <p>{saturation}</p>
      <span className="pt-10">{generatedColor ? generatedColor : "generating..."}</span>
      <div className="flex gap-5">
        <input type="checkbox" id="showHash" checked={showHash} onChange={() => setShowHash(!showHash)} className="cursor-pointer select-none" />
