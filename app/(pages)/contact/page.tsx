...
export default function Page() {
  ...
  async function fakeMessage() {
    ...
    setLoading(false)
    toast("There was an error, please try again later.", "error")
    setFormData({name: "", email: "", message: ""})
  }
    ...
    <main className="min-h-screen max-w-7xl mx-auto md:px-0 px-6">
      ...
      <div className="grid md:grid-cols-2 gap-10 mt-10">
        <div className="flex flex-col relative">
          ...
          <input value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} type="email" id="contact-email" className="bg-woodsmoke-light shadow-(--inset_shadow) border-white/10 light:border-black/10 rounded-xl mt-2 px-4 py-2 focus:outline-none focus:ring-2 ring-lime" />
          <label className="text-sm font-medium mt-5" htmlFor="contact-message">
            Message
          </label>
          <textarea value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} rows={4} id="contact-message" className="bg-woodsmoke-light shadow-(--inset_shadow) border-white/10 light:border-black/10 rounded-xl mt-2 px-4 py-2 focus:outline-none focus:ring-2 ring-lime" />
        ...
        <div className="bg-woodsmoke-light shadow-(--inset_shadow) rounded-3xl p-5 flex flex-col gap-7 light:bg-white h-max">
