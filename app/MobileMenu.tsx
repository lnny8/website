...
const menuData = [
  ...
  },
  {
    ...
    subPages: [
      ...
      { name: "Basismodule", path: "/produkte/produkt-5" },
      { name: "Datenimport", path: "/produkte/produkt-5" },
      { name: "PIM und MAM", path: "/produkte/produkt-5" },
      { name: "Übersetzung", path: "/produkte/produkt-5" },
      { name: "Datenbereitstellunge", path: "/produkte/produkt-5", more: true,
        subsubPages: [{name: "test", path: "/test"}]
      },
    ],
  },
  {
  ...
  },
  {
    ...
    subPages: [
      ...
      { name: "Kundenprojekte", path: "/wissen/kundenprojekte" },
      { name: "Case Studies", path: "/wissen/case-studies" },
  ...
  },
  {
    ...
    path: "/karriere",
    subPages: [
      { name: "Offene Stellen", path: "/karriere/offene-stellen" },
      { name: "Arbeiten bei crossbase", path: "/karriere/arbeiten-bei-crossbase" },
      { name: "Studierende", path: "/karriere/studierende" },
      { name: "Absolventen", path: "/karriere/absolventen" },
    ],
  },
]
...
const MobileMenu = () => {
  ...
  return (
    <div className="w-full h-full absolute z-[100]">
      ...
      <div
        id="menu"
        className="absolute top-[2.5vh] right-5 rounded-lg w-10 h-10 flex flex-col gap-1 justify-center items-center transition-colors duration-1000 z-[200]"
      ...
      <div className={`h-[100dvh] absolute inset-0 top-0 overflow-hidden ${menuOpen ? "" : "pointer-events-none"}`}>
        ...
        <div id="mainMenuContainer" className="absolute inset-0 flex flex-col pt-32 gap-3 pl-12">
          {menuData.map((tab, index) => (
            <div
              ...
              onClick={() => handleTabClick(index, tab.directLink)}
            >
        ...
        <div id="subMenuContainer" className="absolute inset-0 flex flex-col pt-32 gap-5 pl-12">
          ...
          {/* Sub-page title */}
          {activeTabIndex !== null && (
            <h2 className="text-4xl font-bold text-[#180c6c] mb-6">{menuData[activeTabIndex].name}</h2>
          )}
          ...
          {activeTabIndex !== null &&
            menuData[activeTabIndex].subPages.map((subPage, index) => (
              <div
                key={subPage.name}
                id="subTab"
                className="text-2xl text-[#180c6c] font-medium hover:text-[#2d1eb3] transition-colors cursor-pointer"
        ...
        <div id="subSubMenuContainer" className="absolute inset-0 flex flex-col pt-32 gap-5 pl-12">
          ...
          {activeTabIndex !== null && activeSubTabIndex !== null && (
            <>
              ...
              <h3 className="text-3xl font-semibold text-[#180c6c] mb-6">
                {menuData[activeTabIndex].subPages[activeSubTabIndex].name}
              </h3>
            </>
          )}
...
{activeTabIndex !== null &&
  ...
  "more" in menuData[activeTabIndex].subPages[activeSubTabIndex] &&  // Prüfen, ob "more" überhaupt existiert
  menuData[activeTabIndex].subPages[activeSubTabIndex].subsubPages?.map((subSubPage: any) => (
    <Link
      ...
      className="text-2xl text-[#180c6c] font-medium hover:text-[#2d1eb3] transition-colors"
    >
      {subSubPage.name}
    </Link>
  ))}
